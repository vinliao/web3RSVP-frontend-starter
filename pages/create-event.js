...
export default function CreateEvent() {
  ...
  // create the transaction on Polygon and emit event
  // which can be detected by The Graph
  const createEvent = async (cid) => {
    try {
      ...
      if (rsvpContract) {
        ...
        setLoading(true);
        // maybe even play some animation here for a little extra niceness
  ...
  // 1. when user submit form, handle it here
  async function handleSubmit(e) {
    ...
    // create a body, send POST request to /api/store-event-data
    // which stores the stuff on web3.storage
    try {
